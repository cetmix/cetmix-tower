...
- Users can select from predefined values in the "Option" field when options are available for a variable.
- If options exist:

  - The "Value" field becomes synchronized with the selected option.
  - Users cannot manually enter a value but can select from available options.
- If no options exist:

  - The "Option" field is locked for changes.
  - Users can manually input values in the "Value" field.
...
-  **Reference**: Unique identifier used to address variable in
   ...
   based on name
- **Variable Type**: Select the type of the variable.

  - **String**: The default type where the variable has Values.
  - **Option**: Allows the variable to hold a value selected from a predefined list of Options.
