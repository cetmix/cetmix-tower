...
class CxTowerVaultMixin(models.AbstractModel):
    ...
    def _get_secret_values(self, fields_list=None):
        ...
        fields_to_fetch = fields_list or self.SECRET_FIELDS
        if not (self and fields_to_fetch):
            return {}
    ...
    def _set_secret_values(self, vals):
        ...
        for record in self:
            for field, value in vals.items():
                # Find existing vault record using filtered (in-memory operation)
                existing_record = existing_vault_records.filtered(
                    lambda v, record_id=record.id, field_name=field: (
                        v.res_id == record_id and v.field_name == field_name
                    )
                )
    ...
    def unlink(self):
        ...
        Automatically removes all associated vault records before deleting
    ...
    def _extract_and_replace_secret_fields(self, vals_list):
        ...
        Returns:
            tuple: (secret_vals dict mapping temp IDs to secret values,
                    modified vals_list with temp identifiers)
        ...
        for vals in vals_list:
            for secret_field in self.SECRET_FIELDS:
                if secret_field in vals and vals[secret_field]:
                    temp_id_counter += 1
