...
class TestTowerJetTemplate(TestTowerJetsCommon):
    ...
    # ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    #   _is_installation_needed Tests
    # ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

    def test_is_installation_needed_server_not_installed(self):
        """Test _is_installation_needed when server is not installed"""
        ...
        # Template is not installed on server
        result = self.jet_template_test._is_installation_needed(server)
        self.assertTrue(result, "Should return True when server is not installed")
    ...
    def test_is_installation_needed_installation_in_progress_to_install(self):
        """Test _is_installation_needed when installation is in to_install state"""
        # pylint: disable=protected-access
        # Create a server
        server = self.Server.create(
            {
        ...
        # Create an installation record in to_install state
        install_record = self.JetTemplateInstall.create(
            {
                "jet_template_id": self.jet_template_test.id,
                "server_id": server.id,
        ...
        result = self.jet_template_test._is_installation_needed(server)
        self.assertFalse(
            result, "Should return False when installation is in to_install state"
        )
    ...
    def test_install_on_servers_with_dependencies(self):
        """Test install_on_servers with dependency hierarchy"""
    ...
    def test_install_on_servers_complex_dependency_tree(self):
        """Test install_on_servers with complex dependency tree"""
        # pylint: disable=protected-access
        # Use class-level dependency hierarchy
        # Use existing server from common.py
        server = self.server_test_1
