...
class TestDroneCommon(TestTowerBaseCommon):
    ...
    Skills are registered by patching `_get_drone_skills`; the callback is
    a method patched onto `cx.tower.tag`.
    """
    ...
    def setUpClass(cls):
        ...
        def build_payload(controller, **params):
            cls.payload_calls.append((controller.id, params))
            return {"echo": params.get("echo"), "controller": controller.id}

        cls.skills = {
            "test_skill": {
                "build_payload": build_payload,
                "get_timeout": lambda **params: params.get("timeout", 60),
                "get_controller_tags": lambda **params: params.get(
                    "tags", cls.env["cx.tower.tag"]
                ),
            },
            "untagged_skill": {
                "build_payload": build_payload,
                "get_timeout": lambda **params: 0,
            },
        }
        job_class = type(cls.Job)
        original_get_drone_skills = job_class._get_drone_skills

        def _get_drone_skills(self):
            skills = original_get_drone_skills(self)
            skills.update(cls.skills)
            return skills

        cls.startClassPatcher(
            patch.object(job_class, "_get_drone_skills", _get_drone_skills)
        )
        ...
        # Records
        cls.skill = cls.Skill.create({"name": "Test Skill", "code": "test_skill"})
        cls.skill_untagged = cls.Skill.create(
            {"name": "Untagged Skill", "code": "untagged_skill"}
        )
    ...
    # Helpers
    def launch(self, skill="test_skill", target=None, **params):
        target = target or self.target
        return target.launch_drone(
            tower_drone_skill=skill,
