...
class TestDroneCallback(TestDroneCommon):
    def test_callback_truthy(self):
        ...
        self.assertEqual(call["record"], self.target.ids)
    ...
    def test_one_of_several_targets_deleted(self):
        """A deleted id is dropped; the callback runs once on the survivor."""
        first = self.Tag.create({"name": "Survivor"})
        second = self.Tag.create({"name": "Deleted"})
        records = self.Tag.browse([first.id, second.id])
        ...
        self.assertEqual(job.state, "done")
        self.assertEqual(len(self.callback_calls), 1)
        self.assertEqual(self.callback_calls[0]["record"], [first.id])

    def test_every_stored_target_deleted(self):
        """The job fails when every stored id is gone."""
        first = self.Tag.create({"name": "Gone A"})
        second = self.Tag.create({"name": "Gone B"})
