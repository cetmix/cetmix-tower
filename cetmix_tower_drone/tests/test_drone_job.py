...
class TestDroneJob(TestDroneCommon):
    ...
    # ------------------------------
    def test_launch_rejects_unknown_skill(self):
        with self.assertRaises(ValidationError) as ctx:
            self.launch(skill="no_such_skill")
        for code in self.target.get_drone_skills():
    ...
    def test_launch_rejects_non_bound_callbacks(self):
        ...
        for callback in (lambda job, result: True, make_closure(), plain_function):
            with self.assertRaises(ValidationError):
                self.target.launch_drone(
                    tower_drone_skill="test_skill", tower_drone_on_complete=callback
                )

    def test_launch_rejects_empty_and_multi_recordsets(self):
        for records in (self.Tag.browse(), self.tag_a | self.tag_b):
            with self.assertRaises(ValidationError):
                records.launch_drone(
                    tower_drone_skill="test_skill",
                    tower_drone_on_complete=records._test_drone_done,
                )
    ...
    def test_launch_accepts_any_model(self):
        ...
        with patch.object(
            ...
            job = partner.launch_drone(
                tower_drone_skill="test_skill",
                tower_drone_on_complete=partner._drone_partner_done,
            )
        self.assertEqual(len(job), 1)
        self.assertEqual(job.res_model, "res.partner")
        self.assertEqual(job.res_id, partner.id)
        self.assertEqual(job.method_name, "_drone_partner_done")

    def test_get_drone_skills(self):
        self.assertEqual(
            self.target.get_drone_skills(), ["test_skill", "untagged_skill"]
        )
        self.assertEqual(
            self.target.with_user(self.user_user).get_drone_skills(),
            ["test_skill", "untagged_skill"],
        )
        with patch.dict(self.skills, {"another": self.skills["test_skill"]}):
            self.assertEqual(
                self.target.get_drone_skills(),
                ["another", "test_skill", "untagged_skill"],
            )

    def test_no_skill_uses_reserved_param_names(self):
        reserved = {"tower_drone_skill", "tower_drone_on_complete"}
        for skill in self.Job._get_drone_skills().values():
            for func in skill.values():
                self.assertFalse(
                    reserved & set(inspect.signature(func).parameters),
                    "A skill must not use the reserved job argument names",
                )

    def test_skill_params_named_skill_and_on_complete(self):
        """The prefix keeps skill params apart from job arguments."""
        job = self.target.launch_drone(
            tower_drone_skill="test_skill",
            tower_drone_on_complete=self.target._test_drone_done,
            skill="other",
            on_complete="value",
        )
        self.assertEqual(job.sudo().skill_id, self.skill)
        self.run_postcommit()
        self.assertIn(
            {"skill": "other", "on_complete": "value"},
            [params for __, params in self.payload_calls],
        )

    def test_launch_stores_callback_and_nothing_else(self):
        job = self.launch(echo="secret param").sudo()
        self.assertEqual(job.res_model, "cx.tower.tag")
        self.assertEqual(job.res_id, self.target.id)
    ...
    def test_selection_tags(self):
        self.controller_1.tag_ids = [(6, 0, [self.tag_a.id])]
        # No tags from the skill: only untagged controllers match
        self.assertEqual(self.launch().sudo().controller_id, self.controller_2)
        # Intersection matches the tagged controller
        self.assertEqual(
            self.launch(tags=self.tag_a).sudo().controller_id, self.controller_1
        )
        # No intersection: untagged controller still matches
        self.assertEqual(
            self.launch(tags=self.tag_b).sudo().controller_id, self.controller_2
        )
        # Skill without get_controller_tags matches only untagged controllers
        self.controller_2.tag_ids = [(6, 0, [self.tag_b.id])]
        self.assertFalse(self.launch(skill="untagged_skill"))
    ...
    def test_rollback_after_launch_sends_nothing(self):
        with self.registry.cursor() as cr:
            ...
            job = target.launch_drone(
                tower_drone_skill="test_skill",
                tower_drone_on_complete=target._test_drone_done,
            )
            self.assertTrue(job)
    ...
    def test_submission_accepted(self):
        job = self.launch(echo="hello").sudo()
        self.run_postcommit()
    ...
    def test_payload_encrypted_per_candidate(self):
        ...
        for post, controller in zip(
            ...
            self.assertEqual(
                envelope["data"], {"echo": "hello", "controller": controller.id}
            )
            self.assertNotIn(
                controller.sudo()._get_secret_value("drone_api_key"),
                json.dumps(envelope),
            )
        # Payload rebuilt for each candidate
        self.assertEqual(
            [controller_id for controller_id, __ in self.payload_calls],
            [self.controller_1.id, self.controller_2.id],
        )
    ...
    def test_public_methods(self):
        ...
        self.assertEqual(public, {"action_cancel", "launch_drone", "get_drone_skills"})
