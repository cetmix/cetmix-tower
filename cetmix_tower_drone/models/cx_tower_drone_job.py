...
CALLBACK_FIELDS = ("res_model", "res_ids", "method_name", "user_id")
...
class CxTowerDroneJob(models.Model):
    ...
    res_ids = fields.Json(string="Target Records")
    ...
    # ------------------------------
    @api.model
    def _launch(self, skill_reference, on_complete, payload, drone_timeout=0):
        """Validate, reserve a controller and create a pending job.

        Args:
            skill_reference (str): Skill reference. A reference no
                controller has advertised yields an empty recordset.
            on_complete (method): Bound method of a recordset, or of the
                model when the recordset has no ids.
            payload (dict): JSON object sent as ``data``. Never stored.
        ...
        if target is None or not method_name:
            raise ValidationError(
                self.env._(
                    "The completion callback must be a bound method of a recordset."
                )
            )
        if not isinstance(target, models.BaseModel):
            raise ValidationError(
                self.env._("The completion callback must be bound to a recordset.")
            )
        # `.ids` drops a new record with no origin and keeps the origin of a
        # new record created from a saved one. `_ids` still holds the NewId.
        if any(isinstance(record_id, NewId) for record_id in target._ids):
            raise ValidationError(
                self.env._("The completion callback must be bound to saved records.")
            )
        if not isinstance(payload, dict):
            raise ValidationError(self.env._("The drone payload must be a dictionary."))
        try:
            json.dumps(payload, allow_nan=False)
        except (TypeError, ValueError) as exc:
            raise ValidationError(
                self.env._("The drone payload must be JSON-serializable.")
            ) from exc
        if type(drone_timeout) is not int or drone_timeout < 0:
            raise ValidationError(
                self.env._(
                    "The drone timeout must be an integer "
                    "greater than or equal to zero."
                )
            )

        skill = (
            self.env["cx.tower.drone.skill"].sudo().get_by_reference(skill_reference)
        )
        if not skill:
            return self.browse()
        controller = next(
            (
                candidate
                for candidate in self._select_candidates(skill)
        ...
        job = self.sudo().create(
            {
                ...
                "timeout": drone_timeout,
                "res_model": target._name,
                "res_ids": target.ids or False,
                "method_name": method_name,
                "user_id": self.env.uid,
            }
        )
        job._register_submission(payload)
        return job

    @api.model
    def _select_candidates(self, skill, exclude=None):
        """Return controllers that may take a job of this skill.

        Capacity is not checked here: it is reserved by
        ``cx.tower.drone.controller._reserve()``. Tags are not used.
    ...
    def _register_submission(self, payload):
        """Submit the job after the current transaction commits.

        Args:
            payload (dict): JSON object sent as ``data``, held in memory
                only.
        """
        self.ensure_one()
        job_id = self.id
    ...
    def _submit(self, payload):
        ...
        another controller while acceptance is unknown. ``payload`` is the
        same object for every controller tried.

        Args:
            payload (dict): JSON object sent as ``data``.
        """
        self.ensure_one()
        cr = self.env.cr
        skill = self.skill_id
        ...
        while controller:
            ...
            if claim == _NEXT:
                ...
                outcome = self._submit_send(controller, payload)
            ...
            if decision == _STOP:
                return
            controller = self._select_candidates(skill, exclude=tried)[:1]
    ...
    def _submit_send(self, controller, payload):
        """Encrypt and POST the payload to the controller.
    ...
    def _deliver(
        ...
        # An empty recordset is the model and is falsy. Only None is unresolved.
        if target is None:
            job.write(
                {
    ...
    def _get_callback_target(self):
        """Resolve the callback recordset, or the model when no ids are stored.

        Returns:
            recordset: Target as the job user. The model, with no records,
                when no ids are stored. None when the model is missing, the
                method is not callable, or every stored record is gone.
        """
        self.ensure_one()
        ...
        if self.method_name in model._fields or not callable(
            ...
            return None
        if not self.res_ids:
            return model.with_user(self.user_id)
        target = model.sudo().browse(self.res_ids).exists()
        if not target:
            return None
