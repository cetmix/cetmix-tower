...
The controller form has a Commands button. It opens the command logs run on that controller and shows how many there are.

Open the drone job to see Command log. It links to the command log that sent the job, including while that command is still running. The field is hidden when the job was not started from a command.
...
The handler prepares the payload and passes it to `launch_drone()`. A controller that runs `ssh` receives that object as `data`:

- `commands`. The command to run: a string, or a list of strings.
- `sudo`. `n` to run without a sudo password, `p` to run with the password, or `false` when the command does not use sudo.
- `connection`. SSH connection values: `host`, `port`, `username`, `auth_mode`, `password`, `ssh_key`, `host_key` and `skip_host_key`.

## Timeout

The "Command Timeout" setting (`cetmix_tower_server.command_timeout`) is passed as `drone_timeout`. A negative value is sent as 0. It counts differently than for commands run inside Odoo: it is the time without any sign of life from the controller, not the total run time. A command whose controller keeps reporting that it is working can run longer than "Command Timeout". Commands are checked every 15 minutes. When the timeout is reached the command is finished as timed out and the controller is asked to stop the work.
