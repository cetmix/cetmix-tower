...
class TestDroneSshDispatch(TestDroneSshCommon):
    ...
    def test_registration(self):
        """Defer handler and zombie domain are registered."""
    ...
    def test_controller_command_log_action(self):
        log, _job = self.dispatch()
        self.assertEqual(self.controller_1.drone_command_log_count, 1)
        action = self.controller_1.action_view_command_logs()
        self.assertEqual(action["res_model"], "cx.tower.command.log")
        self.assertEqual(
            action["domain"],
            [("drone_controller_id", "=", self.controller_1.id)],
        )
        self.assertIn(log, self.env["cx.tower.command.log"].search(action["domain"]))

    def test_job_command_log_link(self):
        log, job = self.dispatch()
        self.assertEqual(job.command_log_id, log)
    ...
    def test_no_skill_record(self):
        ...
        self.ICP.set_param(NO_CONTROLLER_POLICY_PARAM, "fail")
        log = self.run_command()
        self.assertFalse(log.is_running)
        self.assertEqual(log.command_status, NO_DRONE_CONTROLLER)

    def test_no_controller_policy(self):
        """No controller uses built-in SSH; fail finishes the log."""
    ...
    def test_timeout_from_command_timeout(self):
        """The job stores command timeout, and a negative value as 0."""
        self.ICP.set_param("cetmix_tower_server.command_timeout", "0")
        __, job = self.dispatch()
        self.assertEqual(job.timeout, 0)

        self.ICP.set_param("cetmix_tower_server.command_timeout", "90")
        __, job = self.dispatch(self.command_list_dir)
        self.assertEqual(job.timeout, 90)

        self.ICP.set_param("cetmix_tower_server.command_timeout", "-5")
        __, job = self.dispatch(self.command_create_dir)
        self.assertEqual(job.timeout, 0)

        for value in ("", "not-a-number"):
            self.ICP.set_param("cetmix_tower_server.command_timeout", value)
            __, job = self.dispatch()
            self.assertEqual(job.timeout, 0)
